...
class RecipeListTest
{
    @Test
    fun getRecipes() {
        val recipe_2 = Recipe("cocktail 2", 4.0F, 1)
        val recipe_3 = Recipe("cocktail 3", 4.0F, 1)
        val recipe_1 = Recipe("cocktail 1", 4.0F, 1)
        ...
        recipe_list.forEach {
            ...
            assertNotNull("recipe: rating <float> invalid!", 0.0F < it.rating && it.rating < 5.0F)
            assertNotNull("recipe: times_rated <int> invalid!", it.times_rated >= 0)
        }
    }
}